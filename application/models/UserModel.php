		...
		if ($user) {
			if ($user['is_active'] == 1) {
				if (password_verify($post['pw'], $user['pw'])) {
					...
					$this->session->set_flashdata('message', '<div>Password salah!</div>');
					redirect('auth');
				}
			}else{
				$this->session->set_flashdata('message', '<div>Akun Belum diaktifasi!</div>');
				redirect('auth');
			}
		}else{
			$this->session->set_flashdata('message', '<div>Akun ini belum terdaftar, Silahkan daftar!</div>');
			redirect('auth');
		}
	}
